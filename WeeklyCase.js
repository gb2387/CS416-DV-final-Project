...
if(data1 == null)
  {var data1 = await(d3.csv("https://raw.githubusercontent.com/gb2387/CS416-DV-final-Project/main/CovidCasesDeathsTrend.csv"));
    ...
    state = [...new Set(state)];
    //console.log(state);
    d3.select("#WeeklyCase").append("svg").attr("height",20).append("g").append('text').transition().duration(300).attr("x", 0).attr("y", 10).attr("id","annotation")
...
d3.select("#WeeklyCase").append("div").attr("id","stateDiv").append("label").text("Select a State: ").attr("style","font-size:15px;font-weight:bold;");
d3.select("#WeeklyCase").select("#stateDiv").append("select").attr("id","stateButton").selectAll("myOptions").data(state).enter().append("option")
...
function update(selectedState) {
  ...
  var max = d3.max(data, function(d) { return parseInt(d.Cases.replace(/,/g,'')); });
  // Add Y axis
  //console.log(max);
  ...
  .attr("d", d3.line()
    ...
    .y(function(d) { return y(parseInt(d.Cases.replace(/,/g,''))); })
    );

}

    var svg = d3.select("#WeeklyCase").append("svg")
  ...
  var line = svg.append("path")
    ...
    function drawTooltip(){
        ...
        tooltipLine.attr('stroke', 'black').attr("shape-rendering","crispEdges")
            ...
            "<span style = 'font-size:15px;color:black'><b>" + "Cases:"+ cases + "</b></span></br>";
            div.style("opacity", 1);
            div.html(html)
...
d3.selectAll("svg").append("g").append("text").transition().duration(1000).attr("id","annotation").attr("x", 1002).attr("y", 147)
	.text("slope we can see that in last 6 months people got infected with virus has").attr("font-size", "12px").attr("fill", "black")
d3.selectAll("svg").append("g").append("text").transition().duration(1000).attr("id","annotation").attr("x", 1002).attr("y", 158)
	.text("reduced compared to the number of cases 6 months before the vaccine.").attr("font-size", "12px").attr("fill", "black")
